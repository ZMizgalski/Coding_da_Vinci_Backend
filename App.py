...
import os
import json
from flask import Flask, Response, request, flash, redirect, send_from_directory, send_file
...
NameOfMixedFile = "MixedFile"
secretWorkingDirectoryKey = 'Working'
secretDoneDirectoryKey = 'Done'
...
def renderImage():
    ...
    path = tmpImages + '\\' + date + "_" + secretWorkingDirectoryKey
    makeDir(path)
    ...
    os.rename(path, tmpImages + '\\' + date + "_" + secretDoneDirectoryKey)
    return send_from_directory(tmpImages + '\\' + date + "_" + secretDoneDirectoryKey, mixedFileName, as_attachment=True)
    # file = request.files['file']
    # if file.filename == '':
